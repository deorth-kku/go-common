...
import (
	...
	cstructs "github.com/deorth-kku/go-common/datatypes/structs"
	citer "github.com/deorth-kku/go-common/iter"
)

func SetRaw(f io.Writer, lv slog.Level, format Format, opts ...SlogOption) {
	slog.SetDefault(GetLogger(f, lv, format.NewHandler, opts...))
}

func Set(file string, level string, format string, opts ...SlogOption) (err error) {
	var f *os.File
	if file == "" {
		f = os.Stderr
	} else {
...
func (SlogStruct[T]) convert_struct(_ []string, attr slog.Attr) slog.Attr {
	a := attr.Value.Any()
	if _, ok := a.(T); ok {
		attr.Value = Map2Group(cstructs.MustToMap(a))
	}
	return attr
}
...
func sliceconv[Slice ~[]E, E any](s Slice) citer.Seq2[string, any] {
	return func(yield citer.Yield2[string, any]) {
		for i, v := range s {
...
func Iter2Group(it citer.Seq2[string, any]) slog.Value {
