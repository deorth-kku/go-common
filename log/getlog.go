...
//go:generate stringer -type=Format -linecomment  --trimprefix formatEnd
type Format uint8

func (lf *Format) UnmarshalJSON(data []byte) (err error) {
	...
	*lf, err = ParseFormat(str)
	return
}

func (lf *Format) MarshalJSON() ([]byte, error) {
...
const (
	DefaultFormat Format = iota // DEFAULT
	TextFormat                  // TEXT
	JsonFormat                  // JSON
	formatEnd
)
...
func (f Format) NewHandler(w io.Writer, opts *slog.HandlerOptions) slog.Handler {
	switch f {
	case DefaultFormat:
