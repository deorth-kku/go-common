...
type Float32[T NullTo] float32

func (f *Float32[T]) Init() {
	var t T
	*f = Float32[T](t.NullValue())
}

func (f Float32[T]) IsZero() bool {
	var t T
	return f == Float32[T](t.NullValue())
}

func (f *Float32[T]) UnmarshalJSON(data []byte) error {
	if string(data) == "null" {
		f.Init()
		return nil
	} else {
...
func (f Float32[T]) MarshalJSON() ([]byte, error) {
	switch {
	case cmath.IsNaN(f),
		cmath.IsInf(f, 1),
		cmath.IsInf(f, -1):
...
func (f Float32[T]) Value() (driver.Value, error) {
	switch {
	case cmath.IsNaN(f),
		cmath.IsInf(f, 1),
		cmath.IsInf(f, -1):
...
func (f *Float32[T]) Scan(value any) error {
	switch v := value.(type) {
	case nil:
		f.Init()
	case float32:
...
type Float64[T NullTo] float64

func (f *Float64[T]) Init() {
	var t T
	*f = Float64[T](t.NullValue())
}

func (f Float64[T]) IsZero() bool {
	var t T
	return f == Float64[T](t.NullValue())
}

func (f *Float64[T]) UnmarshalJSON(data []byte) error {
	if string(data) == "null" {
		f.Init()
		return nil
	} else {
...
func (f Float64[T]) MarshalJSON() ([]byte, error) {
	switch {
	case cmath.IsNaN(f),
		cmath.IsInf(f, 1),
		cmath.IsInf(f, -1):
...
func (f Float64[T]) Value() (driver.Value, error) {
	switch {
	case cmath.IsNaN(f),
		cmath.IsInf(f, 1),
		cmath.IsInf(f, -1):
...
func (f *Float64[T]) Scan(value any) error {
	switch v := value.(type) {
	case nil:
		f.Init()
	case float32:
